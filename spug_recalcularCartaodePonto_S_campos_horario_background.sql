...
CREATE PROCEDURE [dbo].[spug_recalcularCartaodePonto_S_campos_horario_background] 
...
BEGIN
	...
	begin
		...
		set @escala_vinculada = (select case when count(fuesccodigo) > 0 then 1 else 0 end from tbgabfuncionarioescala FE (nolock) 
		...
		begin
			...
			begin
				set @func_ativo = 1
				declare @recalcular bit,@escala bit, @vigencia datetime
				...
				begin
					...
					begin
						...
						exec dbo.spug_incluirTotalizadores @acordcodigo, @funcicodigo, @dt, 'spug_recalcularCartaodePonto_S_campos_horario_background'
