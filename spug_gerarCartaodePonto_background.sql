...
-- =============================================
CREATE PROCEDURE [dbo].[spug_gerarCartaodePonto_background] 
...
BEGIN
	...
	begin
		...
		begin
			declare @datademissao datetime
			set @datademissao = (select top 1 funcidatademissao from tbgabfuncionario (nolock) where funcicodigo = @funcicodigo order by funcidatademissao desc)
			
			-- SITUAÇÃO DO FUNCIONÁRIO SE ENCONTRA DIFERENTE DE ATIVO
			if @sit_f = 'D' and @datademissao is not null and @datademissao <> '1900-01-01 00:00' and  @datademissao < @periodoiniciodatabase
			begin 
			...
			begin
				...
				begin
					...
					select @cartacodigo=cartacodigo from tbgabcartaodeponto (nolock) where cartadatajornada = @dt and funcicodigo = @funcicodigo
					...
					begin
						...
						try
							...
							afdtgcodigo_e1,afdtgcodigo_s1,afdtgcodigo_e2,afdtgcodigo_s2,afdtgcodigo_e3,afdtgcodigo_s3,afdtgcodigo_e4,afdtgcodigo_s4,
							cartausuaralteracao,cartadataalteracao,cartahorarreferencia
							) 
							values (
							...
							@horainterjornadacomplsdesc,@afdtgcodigo_e1,@afdtgcodigo_s1,@afdtgcodigo_e2,@afdtgcodigo_s2,@afdtgcodigo_e3,@afdtgcodigo_s3,@afdtgcodigo_e4,@afdtgcodigo_s4,
							@usuarcodigo,getdate(),@h_referencia)
						...
						begin catch
							...
							values (@funcicodigo,@dt,'spug_gerarCartaodePonto_background',@usuarcodigo,ERROR_MESSAGE())
						end catch;
					end 
					...
					begin
						...
						select top 1 @horarcodigo=horarcodigo,@cartadesconsiderapreassinalado=cartadesconsiderapreassinalado,@h_referencia=cartahorarreferencia 
	...
	begin 
		set @obs = @obs + 'Sem movimentação ou centro de custo bloqueado;';set @haserror = 1;
	end

	if (select top 1 escala_vinculada from @cartaodeponto) = 0
	begin 
		set @obs =  @obs + 'Sem escala vinculada;';set @haserror = 1;
	end

	if (select top 1 func_ativo from @cartaodeponto) = 0
	begin 
		set @obs = @obs + 'Funcionário inativo;';set @haserror = 1;
	end
